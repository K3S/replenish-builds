...
-- AR_FINLOG
CL: CRTSQLRPGI OBJ(ACS_5OBJ/AR_FINLOG) SRCFILE(ACS_5DEV/QRPGLESRC) COMMIT(*NONE) OPTION(*EVENTF *XREF) DBGVIEW(*SOURCE) TGTRLS(V7R3M0);
CL: GRTOBJAUT OBJ(ACS_5OBJ/AR_FINLOG) OBJTYPE(*PGM) USER(*PUBLIC) AUT(*ALL);
...
-- APIs
CL: CALL PGM(DEVTOOLS/APIPRART) PARM('AR_FINLOG' 'ACS_5DEV' 'QRPGLESRC' 'QRPGLESRC' 'WEB_5TDV' 'WEB_5TST' 'QSRVSRC' 'K3SDIR');
...
-- APIs
CL: CALL PGM(DEVTOOLS/APIPRARD) PARM('AR_FINLOG' 'WEB_5TDV' 'QRPGLESRC' 'QRPGLESRC' 'WEB_5DEV' 'WEB_5OBJ' 'QSRVSRC' 'K3SDIR');
...
CL: SAVOBJ OBJ(AR_FINLOG) LIB(WEB_5TST) DEV(*SAVF) SAVF(QGPL/k3shotfix);
