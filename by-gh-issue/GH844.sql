...
-- AC_PSIREPT
CL: CPYSRCF FROMFILE(ACS_5DEV/QCLLESRC) TOFILE(WEB_5TDV/QCLLESRC) FROMMBR(AC_PSIREPT) TOMBR(AC_PSIREPT);
CL: CRTBNDCL PGM(WEB_5TST/AC_PSIREPT) SRCFILE(WEB_5TDV/QCLLESRC) OPTION(*EVENTF) DBGVIEW(*SOURCE);
CL: GRTOBJAUT OBJ(WEB_5TST/AC_PSIREPT) OBJTYPE(*PGM) USER(*PUBLIC) AUT(*ALL);
...
-- AC_PSIREPT (ready)
CL: CPYSRCF FROMFILE(WEB_5TDV/QCLLESRC) TOFILE(WEB_5DEV/QCLLESRC) FROMMBR(AC_PSIREPT) TOMBR(AC_PSIREPT);
CL: CRTBNDCL PGM(WEB_5OBJ/AC_PSIREPT) SRCFILE(WEB_5DEV/QCLLESRC) OPTION(*EVENTF) DBGVIEW(*SOURCE);
CL: GRTOBJAUT OBJ(WEB_5OBJ/AC_PSIREPT) OBJTYPE(*PGM) USER(*PUBLIC) AUT(*ALL);

-- AR_PSIREPT (ready)
CL: CPYSRCF FROMFILE(WEB_5TDV/QRPGLESRC) TOFILE(WEB_5DEV/QRPGLESRC) FROMMBR(AR_PSIREPT) TOMBR(AR_PSIREPT);
CL: CRTSQLRPGI OBJ(WEB_5OBJ/AR_PSIREPT) SRCFILE(WEB_5DEV/QRPGLESRC) COMMIT(*NONE) OPTION(*EVENTF *XREF) DBGVIEW(*NONE) TGTRLS(V7R3M0);
CL: GRTOBJAUT OBJ(WEB_5OBJ/AR_PSIREPT) OBJTYPE(*PGM) USER(*PUBLIC) AUT(*ALL);

-- Set to current date
CL: CHGDTAARA DTAARA(WEB_5OBJ/AR_RELEASE *ALL) VALUE('031822');
...
CL: SAVOBJ OBJ(AC_PSIMIS AR_INVRLOG AS_SSTOCK AS_PRDINVT AR_PRDINVT AC_PRDINVT AR_INVLOG AC_INVLOG AC_PSIREPT AR_PSIREPT) LIB(WEB_5OBJ) DEV(*SAVF) SAVF(QGPL/k3shotfix);
