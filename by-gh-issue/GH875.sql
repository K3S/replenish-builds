...
-- AC_ACTYOUR
CL: CRTBNDCL PGM(ACS_5OBJ/AC_ACTYOUR) SRCFILE(ACS_5DEV/QCLLESRC) OPTION(*EVENTF) DBGVIEW(*SOURCE);
CL: GRTOBJAUT OBJ(ACS_5OBJ/AC_ACTYOUR) OBJTYPE(*PGM) USER(*PUBLIC) AUT(*ALL);
...
-- CL Wrappers
CL: CALL PGM(DEVTOOLS/APIPRACT) PARM('AC_ACTYOUR' 'ACS_5DEV' 'QCLLESRC' 'QCLLESRC' 'WEB_5TDV' 'WEB_5TST' 'QSRVSRC' 'K3SDIR');
...
-- CL Wrappers
CL: CALL PGM(DEVTOOLS/APIPRACT) PARM('AC_ACTYOUR' 'WEB_5TDV' 'QCLLESRC' 'QCLLESRC' 'WEB_5DEV' 'WEB_5OBJ' 'QSRVSRC' 'K3SDIR');

-- Set to current date
CL: CHGDTAARA DTAARA(WEB_5OBJ/AR_RELEASE *ALL) VALUE('060622');
...
CL: SAVOBJ OBJ(AC_ACTYOUR) LIB(WEB_5TST) DEV(*SAVF) SAVF(QGPL/k3shotfix);
