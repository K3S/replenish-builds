...
-- API Program(s)
-- AR_<api_name>
CL: CRTSQLRPGI OBJ(ACS_5OBJ/<api_name>) SRCFILE(ACS_5DEV/QRPGLESRC) COMMIT(*NONE) OPTION(*EVENTF *XREF) DBGVIEW(*SOURCE) TGTRLS(V7R2M0) TEXT('<api_text>');
...
CL: CRTBNDCL PGM(WEB_5OBJ/AC_<api_name>) SRCFILE(WEB_5DEV/QCLLESRC) OPTION(*EVENTF) DBGVIEW(*SOURCE) TEXT('<api_text>');

-- TS_<api_name>
CL: CPYSRCF FROMFILE(WEB_5TDV/QCLLESRC) TOFILE(WEB_5DEV/QCLLESRC) FROMMBR(TS_<api_name>) TOMBR(TS_<api_name>);
CL: CRTBNDCL PGM(WEB_5OBJ/TS_<api_name>) SRCFILE(WEB_5DEV/QCLLESRC) OPTION(*EVENTF) DBGVIEW(*SOURCE) TEXT('<api_test_text>');
